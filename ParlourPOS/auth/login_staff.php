...
if (isset($_SESSION['user_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] === 'Receptionist') {
        header("Location: " . $base_url . "/staff/cashier/appointments.php");
        exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        $stmt = $conn->prepare("SELECT id, name, password, role, status FROM users WHERE email = ? AND role IN ('Beautician', 'Receptionist') LIMIT 1");
        ...
        if ($user && password_verify($password, $user['password'])) {
            if ($user['status'] === 'Active') {
                ...
                $redirect = ($user['role'] === 'Receptionist') ? '/staff/cashier/appointments.php' : '/staff/beautician/dashboard.php';
                header("Location: " . $base_url . $redirect);
                exit;
            } else {
                $_SESSION['login_error'] = "Account disabled. Please contact Admin.";
            }
        } else {
            $_SESSION['login_error'] = "Invalid Staff credentials.";
        }
        $stmt->close();
    }
...
<head>
    ...
    <title>Staff Terminal | Crafting Styles</title>
    ...
    <style>
        ...
        .hero-bg {
            background-image: url('https://images.unsplash.com/photo-1527799820374-dcf8d9d4a388?auto=format&fit=crop&q=80');
        ...
        .line-input:focus { border-bottom-color: #fecdd3; outline: none; box-shadow: 0 4px 6px -6px #fecdd3; }
...
<body class="relative min-h-screen font-sans antialiased text-white flex">
    <div id="loader-overlay" class="fixed inset-0 bg-[#1A1A1A] flex flex-col items-center justify-center z-[9999]">
        <div class="skeleton-pulse text-center">
            <h1 class="font-serif text-4xl tracking-[0.2em] text-[#fecdd3] mb-2">CRAFTING STYLES</h1>
            <p class="font-sans text-xs tracking-[0.4em] text-gray-500 uppercase">Staff Terminal</p>
        </div>
    </div>
    ...
    <div class="relative z-10 flex-1 hidden lg:flex flex-col justify-center px-16 xl:px-24">
        <h1 class="text-7xl xl:text-9xl text-white font-serif drop-shadow-2xl text-glow">CRAFTING<br><span class="italic text-[#fecdd3]">Styles</span></h1>
        <p class="text-sm tracking-[0.2em] text-gray-100 font-medium mt-8 max-w-lg leading-relaxed uppercase drop-shadow-lg text-glow">
            Staff Terminal &bull; Access Point of Sale and Daily Appointments.
        </p>
    </div>

    <div class="w-full lg:w-[480px] min-h-screen bg-[#900020]/80 backdrop-blur-2xl border-l border-white/10 flex flex-col justify-center px-8 sm:px-12 relative z-20 shadow-2xl">
        <div class="text-left mb-10">
            <div class="lg:hidden mb-10">
                <h1 class="text-4xl text-white font-serif drop-shadow-lg text-glow">CRAFTING <span class="italic text-[#fecdd3]">Styles</span></h1>
                <p class="text-[10px] tracking-[0.3em] uppercase text-white/70 mt-2 drop-shadow-lg">Staff Terminal</p>
            </div>
            <h2 class="font-sans font-medium tracking-widest text-2xl text-white uppercase">Shift Login</h2>
            <p class="text-xs text-gray-300 font-light mt-2 tracking-wide">Beauticians & Receptionists</p>
        </div>

        <form method="POST" action="login_staff.php" class="space-y-8" onsubmit="document.getElementById('loader-overlay').style.display='flex'; setTimeout(()=>document.getElementById('loader-overlay').style.opacity='1',10);">
            ...
            <div class="relative">
                ...
                <input type="email" name="email" required placeholder="Staff Email" class="line-input w-full pl-8 pr-4 py-3 text-sm">
            </div>
            ...
            <button type="submit" class="w-full mt-8 bg-[#fecdd3] hover:bg-white text-black font-bold tracking-[0.2em] text-xs py-4 transition-all shadow-[0_0_15px_rgba(254,205,211,0.2)] uppercase">Login</button>
